...
- The command keeps the reviewer read-only through the isolation arguments of Codex and Grok, and validates its output. Claude and Cursor have no read-only sandbox: Claude runs with `Edit` and `Write` disallowed in an out-of-tree runtime, Cursor isolates only its configuration and session, and for both read-only is enforced afterwards by checking the Git-visible state of the target worktree, which does not detect writes outside it or to ignored paths inside it.
