...
- 改文件任务使用独立任务分支及 `<仓库根目录>/worktrees/<task-name>/` 专用 worktree.
  ...
  `<task-name>` 同分支名, 短 kebab-case. 任务组的组 worktree 是例外: 它按组 ID 命名, 而其分支带 `group/` 前缀, 见 `KANDER-TASK-GROUP-RULES.md`.
...
- 集成前核对上述授权及用户、项目已有的 PR、验收和暂停要求. 无适用授权或门禁未满足时保留分支和 worktree, 报告具体待处理事项; 看板单卡留在 `working/`, 任务组按组规则保留实际状态.
- 源分支是创建任务分支时记录的交付目标, 看板卡记在卡片的 `IMPLEMENTATION` 中, 其他任务记在任务自己的交付记录中: 单卡为 `develop`, 组内任务分支为对应组分支, 组分支为 `develop`. 只能集成到该目标, 不因当前检出分支或临时 rebase 改变目标.
...
- 适用审核是集成前的一次性门; 审核期间 base 冻结. 对单卡, 审核完成后因源分支前进而 rebase 只重做验证; 仅当用户明确要求或手工解决了实质性代码冲突时才重审, 无冲突或冲突仅在 Markdown 文档时不重审. 对任务组, 绑定的收尾证据只在 rebase 后的组分支完整补丁与已关闭的审核补丁相同时才接受, 因此集成 rebase 必须干净应用且补丁不变; 任何冲突 (包括 Markdown) 或补丁变化都按 `KANDER-TASK-GROUP-RULES.md`「合回与清理前置」处理, 组批次的 base 链与修复轮次遵循组级审核规则.
...
- 用户或项目要求 PR 时遵从其审核、CI、合并方式和授权要求, 不以直接集成绕过. PR 已 merged、目标为记录的源分支且最终交付确实包含在合并结果中, 才算集成完成; squash/rebase 合并以这些证据确认, 不要求合并前 commit 仍为目标分支祖先. 对任务组, 持久化收尾证据还需要 `KANDER-TASK-GROUP-RULES.md`「合回与清理前置」所述的 Git 映射.
